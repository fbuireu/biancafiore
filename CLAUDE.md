...
- **Effect**: `Context.Tag` + `Layer` clients for cms/db/email, and `LoggingLive` for the shape of a log line. ADR 0004, ADR 0020
...
**This section names where each runtime is pinned and never what the pin says**, and the failures that led here are worth keeping. Asserting the digit against the manifest fails every bot pull request on a documentation line the bot cannot edit, which is what contribKit did until it stopped. Quoting the digit without asserting it rots, which is what this section did: it said Node 26.5.1 and pnpm 11.15.1 for a while after the manifest had moved. Read `engines.node` or `.nvmrc`.

What *is* asserted is the part a bump cannot rot: each runtime is pinned exactly once. `.nvmrc` and `engines.node` are the same fact written twice and must agree, and no workflow may pin either again, since `prepare-env` reads `node-version-file: .nvmrc` and `pnpm/action-setup` reads `packageManager`. A bot that bumps the manifest moves both together, so those rules never fire on a routine update. A last rule reaches the sentences this section cannot see: no document outside the ADRs names a runtime or a framework beside a version, and the lines here that narrate the stale digits by number are allow-listed by name, because history is not a claim about the tree.
...
**A `:changed` variant names a literal base, and computing one is what it must not do.** A `package.json` script runs under `cmd` on Windows, where `$(...)` is not substituted but passed through as literal argv, so a script that resolved the branch's push target broke every push from a Windows checkout. `test:ut:changed` therefore takes `origin/main` outright: on a branch that is wider than the push needs and never narrower, so it errs safe. Biome's `--changed` is a different story and is left alone, because it diffs against `vcs.defaultBranch`, which is `main`, so standing on `main` there is nothing to compare and `pnpm format:changed` answers *Checked 0 files* however much has changed. Setting `defaultBranch` to a revision expression works (`@{push}` resolves) and is worse: on a branch with no upstream it silently checks nothing and exits zero. Reach for `format:all`, which reads this tree in under a second.
...
**Coverage has a floor of 85 on every metric.** [`vitest.config.ts`](./vitest.config.ts) declares `thresholds` from one `MIN_THRESHOLD` const, the same shape and the same number every sibling repository uses, so a drop below it fails `verify` instead of being noticed in a report nobody opens. Branches is the tight metric here, as it is everywhere: statements and lines sit far above the floor, so the number that will actually stop a push is the one a half-covered conditional moves.
...
- **No Biome suppressions.** Fix the root cause (e.g. reorder selectors) instead of `biome-ignore`; suppress only if truly irreplaceable. Biome: 120 line width; `noConsole` error, exempted for exactly one path, the log transport in `@infrastructure/logging/logger.ts`, where `console` *is* the transport (ADR 0020); everywhere else log through `LoggerService` or the `logger` import. organizeImports on. `noConsole` is *not* part of Biome's recommended preset, so deleting that entry does not tighten it, it silently turns the rule off. `public/**` is excluded from Biome.
...
**[`docs/wiki/`](./docs/wiki) is published, and it is the shape rather than the rules.** [`sync-wiki.yml`](./.github/workflows/sync-wiki.yml) rsyncs it into the repository's GitHub wiki on every push to `main` that touches it, `--delete` included, so the wiki is a mirror and editing a page in the GitHub UI is a change the next sync throws away. The same workflow, byte for byte, publishes the sibling repositories' wikis. Rules follow from where those pages end up, and the docs test asserts every one of them: a **filename is the page name** (`Getting-Started.md` is reachable as `Getting-Started`), a link to another page is that **bare name and never a path**, and a link into this repository is an **absolute `https://github.com/` URL**, because a wiki page has no relative route back into the tree. What a page must not do is restate a rule the tree already states: the normative statement lives beside the code, a wiki copy of it is another one nothing checks, and the page's job is to say where it is.

Propose an ADR in [`docs/adr/`](./docs/adr/) when a decision is **hard to reverse**, **surprising without context** and **the result of a real trade-off**. All of them, or it is not an ADR. Copy [ADR 0000](./docs/adr/0000-adr-template.md), the template, and number it one above the highest existing file (`NNNN-kebab-title.md`, `# N. Title` / `Date:` / `## Status` / `## Context` / `## Decision` / `## Consequences`), then link it from wherever it bites: a Gotchas bullet here, a nested guide, a [`CONTEXT.md`](./CONTEXT.md) entry. There is no separate index; an ADR nothing links to will not be read, which is why both the template and the incoming link are asserted.

Traps worth naming, because each has already happened here: deleting a resolved entry from a "known inconsistencies" or gotchas list is part of the fix, not tidying to do later; and a `file.ts:123` citation silently rots the moment anything above it moves, so prefer naming the symbol.
...
- **Analytics are consent-gated, two different ways:** GA/GTM load on every page with `analytics_storage` denied until the visitor accepts the `analytics` category, set by an inline script in `<head>` before either initialises; nothing in the code makes that requirement visible, so do not reorder or "clean up" that script. **Better Stack's browser tag has no consent mode**, so it is gated by not existing: `updatePreferences` appends it only once `acceptedService("betterstack", "analytics")` is true, and `config.ts` calls that on `onConsent` **and** `onChange`. `onConsent` replaced `onFirstConsent` on purpose, since that one fires only the first time an answer is given and a returning visitor who already accepted would get no tag. `BETTER_STACK_TRACKING_TOKEN` is public by construction, is not the credential the log export uses, and is a **repository** variable: one RUM source serves both stages, and the `environment` the tag is `init`-ed with, derived from the hostname, is the only thing separating preview traffic from real. **Cloudflare Web Analytics is the exception and nothing here can gate it**: the zone injects `beacon.min.js` at the edge before any of this runs, so the only switch is the dashboard; it is kept on purpose, the privacy policy names it, and `securityHeaders.ts` must keep `https://static.cloudflareinsights.com` in `script-src` or the browser blocks it with a console error and no other symptom. ADR 0013, ADR 0020.
...
**Logs and traces leave Cloudflare through its own export, and `console` is what carries the app's lines into it.** `[observability]` alone turns on Workers Logs, which is why the dashboard reported Workers Traces as disabled while observability was on; `[observability.traces]` asks for them, and `destinations` is what gets either off the platform, naming an OTLP endpoint and token configured in the account's dashboard. Every block declares `redact_query_string = true`. The app's own lines go through the `logger` in [`@infrastructure/logging/logger.ts`](./src/infrastructure/logging/logger.ts), one JSON object per call written with `console[level]`, because the export attributes console output to the active span and stamps its trace id: a sink posted to over HTTP from inside the Worker is invisible to the runtime, and its lines land beside the spans they belong to without joining them. That file and its `contract.ts` are byte for byte what contribKit and forever-pto carry, bar the service name and the import alias, so one Better Stack query reads all three. ADR 0020, and ADR 0004 for why the Effect seam over it is a tag.

**A destination belongs to the account, not to the Worker, which is why the stage is in its name.** `biancafiore-web-production-logs`, `biancafiore-web-production-traces` and the `-development` pair sit in one namespace with every other Worker on the account, under the `<repo>-<package>-<stage>-<signal>` shape both siblings use. A destination has no environment of its own, so a development Worker naming production's is accepted, exports happily, and files preview traffic with the live site's, where only `script` tells them apart. **The top level is production's twin**, block for block, so a bare `wrangler deploy` cannot reach the development source. **They must exist in the dashboard before a deploy**, nothing here can assert that they do, and a `destinations` entry naming one that does not is the failure to remember. Rotating the log sink is a dashboard change: no redeploy, and no credential anywhere in this tree.

**Sampling is `1`, and it is written down rather than defaulted.** The rate is the one setting here that loses data with no symptom, and none of these sites serves enough traffic for a rate that drops four errors in five to save anything: both siblings settled on the same number, forever-pto after a spell at `0.2`, so if it ever changes it changes in all three. `docs/docs-consistency.test.ts` asserts the stage split, the exported names, that the top level mirrors production, that the two stages differ in nothing but their destinations, and that nothing in `src` mentions `BETTER_STACK`.

**`console` is the transport, so the `noConsole` exemption is structural.** `biome.json` scopes it to `logger.ts` and nothing else, exactly as both siblings do, and it must not widen: everywhere else in `src` a program that logs takes `LoggerService` in its `R` and says so in its return type, which is what turns an unintended log into a compile error. `500.astro` and `getImagePlaceholders` reach the same object through the plain import, because neither has a layer to provide one.
...
**The cases in [`e2e/smoke.spec.ts`](./e2e/smoke.spec.ts) carry `@smoke` and the step passes no `--pass-with-no-tests`, which is the point.** Playwright exits 1 on an empty set, so the flag would make a typo in the grep green; without it, a grep that stops matching fails the job, which is the only thing that keeps the set honest. They are the cheapest things that prove the Worker is answering rather than merely deployed: the homepage with a non-empty title, an unknown path answering 404, and `robots.txt`. **That set is the same in every repository that deploys**, written the same way and with the same titles, so a set that differs between them is drift rather than a decision; contribKit carries an extra one because `/user/<name>.svg` cannot be prerendered there and nothing else separates a running Worker from a bucket of assets. A smoke case can only assert what the deploy it follows has already published, which is why none of them names a feature: they check that the Worker routes and responds, and the feature coverage stays in the preview suite. They run against the preview too, since `test:e2e` runs the whole directory, and they survive `HIDE_CHROME` because none of them asserts page content.
