...
const Breadcrumbs = ({ location, customBreadcrumb = null, classNames }) => {
  ...
  const breadcrumbs = HOMEPAGE_PATH.concat(location.pathname.split(`/`).
    ...
    <nav aria-label={`Breadcrumb`}
      ...
      <ol className={`breadcrumbs__list`}>
        {breadcrumbs.map((breadcrumb, index) => {
          ...
          if (customBreadcrumb?.position - 1 ===
            index) breadcrumb = customBreadcrumb.label
          ...
          return <>
            {(index !== lastPage) ? (
              <li key={breadcrumb} className={`breadcrumb__item`}>
                <Link to={linkPath}
                      className={`breadcrumb__item__link`}>{breadcrumb}</Link>
                <span aria-hidden={true}
                      className={`breadcrumb__item__separator`}>/</span>
              </li>
            ) : (
