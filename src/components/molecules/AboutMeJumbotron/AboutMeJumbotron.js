...
const AboutMeJumbotron = ({
  ...
  return (
    <section className={`about-me__jumbotron__wrapper wrapper`}>
      ...
      <Markdown
        className={`about-me__jumbotron__header`}>{welcomeText}</Markdown>
      <Markdown
        className={`about-me__jumbotron__description`}>{welcomeDescription}</Markdown>
      ...
      <ul className={`about-me__jumbotron__social-networks__list`}>
        {availableSocialNetworks.urls.map((url, index) => {
          ...
          return <li key={name}
            ...
            <a className={`about-me__jumbotron__social-network__link`}
