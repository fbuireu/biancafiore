...
const FeaturedImageArticleCard = (article) => {
  ...
  return (
    <article
      className={`featured-image-hit-card__item ${
        article?.content?.isFeaturedArticle ? `--is-featured` : ``
      }`}
    >
      <GatsbyImage
        image={
          article?.content?.featuredImage?.childImageSharp?.gatsbyImageData
        }
        className={`hit-card__featured-image`}
      />
      <div className={`hit-card__information`}>
        <span className={`hit-card__information__reading-time`}>
      ...
      <Link to={`/blog${article.fields.slug}`} className={`hit-card__link`}>
        <HitTitle hit={article} attribute={`content.title`}/>
      </Link>
      <div className={`hit-card-subtitle`}>
        <Link
          to={`/tags/${slugify(article.author)}`}
          className={`hit-card-subtitle__author`}
        >
          <Highlight attribute={`author`} hit={article} tagName={`mark`}/>
        </Link>
        <time
          dateTime={article.content.publishDate}
          className={`hit-card-subtitle__publish-date`}
        >
          {localizeDate({ date: article.content.publishDate, locale })}
        </time>
      </div>
      <Summary
        summary={article.content.summary ?? article.excerpt}
        classNames={`hit-card__summary`}
      />
      <Link to={`/blog${article.fields.slug}`} className={`hit-card__cta`}>
        Read more
      </Link>
    </article>
  )
