...
const FeaturedImageArticleCard = article => {
  ...
  return (
    <article
      className={`featured-image-hit-card__item ${article?.content?.isFeaturedArticle
        ? `--is-featured`
        : ``}`}>
      <GatsbyImage
        image={article?.content?.featuredImage?.childImageSharp?.gatsbyImageData}
      ...
      <div className={`hit-card-subtitle`}>
        ...
        <time dateTime={article.content.publishDate}
