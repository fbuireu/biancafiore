...
const ArticleHitCards = () => {
  const FilterResults = connectStateResults(
    ({ searchResults, children }) => searchResults && searchResults.nbHits !== 0
      ? children
      : <p
        className={`filter__results --no-results-found`}>{`Blimey! You're so picky that not even a genie could find what you're looking for. Try to expand your search.`}</p>,
  )
