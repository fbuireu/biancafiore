...
Not every concept needs all of them. `rules.ts` is the exception rather than the norm: it exists for `article`, `breadcrumb`, `city`, `contact`, `tag` and no one else; `contact` carries a schema and the one rule that decides when two addresses are the same person, `breadcrumb` is rules + types, and the rest are schema + types. Add a file when there is something to put in it.
