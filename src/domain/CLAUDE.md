...
- **Rules are named after the domain, not the caller.** `getReadingTime`, `deriveDescription`, `deriveVariant`, `sortFavoriteFirst`, `resolveSlugCollisions`, `buildTagIndexBuckets`, `createPeriod`, `normalizeEmail`: the DTO calls them, but they read as editorial rules, not as mapper helpers.
