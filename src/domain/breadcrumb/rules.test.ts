import { breadcrumbDTO } from "@domain/breadcrumb/rules";
import { describe, expect, it } from "vitest";

describe("breadcrumbDTO.create", () => {
	it("shows no trail at all on the home page", () => {
		expect(breadcrumbDTO.create({ currentPath: "/" })).toEqual([]);
	});

	it("prepends Home to every path that is not the home page", () => {
		expect(breadcrumbDTO.create({ currentPath: "/projects" })).toEqual([
			{ label: "Home", link: "/" },
	...
	it("builds one cumulative link per segment of a nested path", () => {
		expect(breadcrumbDTO.create({ currentPath: "/articles/my-first-post" })).toEqual([
			{ label: "Home", link: "/" },
	...
	it("treats a trailing slash as the same trail as the path without one", () => {
		expect(breadcrumbDTO.create({ currentPath: "/tags/long-form/" })).toEqual(
			breadcrumbDTO.create({ currentPath: "/tags/long-form" }),
		);
	});

	it("turns each slug into title-cased words for the label while the link keeps the slug", () => {
		const [, legal] = breadcrumbDTO.create({ currentPath: "/terms-and-conditions" });
	...
	it("capitalises a word that follows a number without touching the number", () => {
		expect(breadcrumbDTO.create({ currentPath: "/articles/2024-recap" }).at(-1)?.label).toBe("2024 Recap");
	});

	it("drops empty segments left by repeated slashes, collapsing them in the links too", () => {
		expect(breadcrumbDTO.create({ currentPath: "//articles//long-form" })).toEqual([
			{ label: "Home", link: "/" },
			{ label: "Articles", link: "/articles" },
	...
	it("still offers a way back Home when the path is empty", () => {
		expect(breadcrumbDTO.create({ currentPath: "" })).toEqual([{ label: "Home", link: "/" }]);
	});

	it("ignores segments that are only whitespace", () => {
		expect(breadcrumbDTO.create({ currentPath: "/ /articles" })).toEqual([
			{ label: "Home", link: "/" },
			{ label: "Articles", link: "/articles" },
		]);
	});
	...
	it("keeps the deepest segment last, which is what the component renders as the current page", () => {
		const trail = breadcrumbDTO.create({ currentPath: "/tags/writing" });
