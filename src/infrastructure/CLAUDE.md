...
- `persistence.ts` — duplicate check and insert, both requiring `Database`, both stated in domain terms: the address to look up, and the submission to write. Query construction is not here — `findContactByEmail` and `insertContact` are the whole of what this file knows about the database, and `drizzle-orm` is not imported. What is here is the id and the two timestamps a row gets, and the decision that a unique constraint violation means a duplicate: `saveContact` catches `DatabaseError` and asks `isUniqueConstraintViolation` (`db/constraints.ts`) about its `cause`. **That predicate walks the whole `cause` chain, not just the immediate one.** Drizzle rethrows every driver rejection as a `DrizzleQueryError` carrying the real `LibsqlError` as its own `cause`, so by the time `DatabaseLive` packs it into `DatabaseError.cause`, the `LibsqlError` sits one level down. Test only the top level and the `DuplicateContactError` arm of `saveContact` never fires under drizzle at all. **No HTTP status hangs on that arm**: the action wraps `saveContact` in a catch-all that swallows everything it can fail with, so the loser of a duplicate race is answered `ok` either way — it gets its mail out, and only its row is lost. What the walk buys is narrower and still real. The one log line then reads `You already contacted…` instead of drizzle's `Failed query:` dump, which is how an operator tells a row lost to the race from a database that is down; and `saveContact` keeps the failure type it advertises, since the walk is the only thing that ever makes the promised `DuplicateContactError` appear — narrow that catch-all to `DatabaseError` and the race starts answering `UNAUTHORIZED` with nothing in this folder needing to change. The tag is nowhere near dead either way — `checkDuplicatedEntries` raises it from a plain lookup hit, before any mail is sent, and *that* one reaches `contactErrorResponse` uncaught. The walk still matches an unwrapped `LibsqlError`, and visits each link once so a chain that loops back on itself cannot hang it.
...
- `db/constraints.ts` — `isUniqueConstraintViolation`, the one place that knows a `LibsqlError` code. It is a plain predicate rather than a layer method so it can be unit-tested against real driver errors, which no query on `DatabaseLive` can be: the layer itself builds against fake credentials in `db/client.test.ts`, because `createClient` is lazy, but every query it then issues would leave the process. `email/server.test.ts` builds `EmailClientLive` the same way and for the same reason: it asserts the shape of the interface, never a send.
