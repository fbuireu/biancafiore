...
- [`email.tsx`](./utils/email.tsx): `sendEmail`, requiring `EmailClient`, plus one function that is not an Effect at all: `createEmail`, which is `async` because it renders React Email to html and text. `sendEmail` lifts it with `Effect.promise`, so a render that throws is a *defect*, not an `EmailError`: it never reaches the tag switch and the visitor gets the generic 500. That is the intended answer (a template that cannot render is our bug, not the visitor's), but it is why no typed error covers it.
