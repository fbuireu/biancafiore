...
- `persistence.ts` — the duplicate check and the insert, both requiring `Database`, both stated in domain terms: the address to look up, and the submission to write. Query construction is not here; `drizzle-orm` is not imported. What is here is the id and the two timestamps a row gets, and **the whole of the anti-spam policy**, which is two questions asked concurrently about the normalised address:
...
- `email.tsx` — `sendEmail`, requiring `EmailClient`, plus two functions that are not Effects at all: `normalizeEmail`, and `createEmail`, which is `async` because it renders React Email to html and text. `sendEmail` renders and hands the result to `sendContactNotification`; what the mail *is* addressed to is the client's, what it *says* is this file's. It lifts the render with `Effect.promise`, so a render that throws is a *defect*, not an `EmailError`: it never reaches the tag switch and the visitor gets the generic 500. That is the intended answer — a template that cannot render is our bug, not the visitor's — but it is why no typed error covers it.
