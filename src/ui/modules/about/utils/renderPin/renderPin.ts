...
export function renderPin({
    markerData,
    cities,
}: RenderPinParams): HTMLElement {
    const citiesInitialPosition = cities.map(
        ({ data }) =>
            (document.querySelector(`#${slugify(data.name)}`) as HTMLElement)
                ?.offsetTop
    );

    const markerWrapper = document.createElement("button");
    markerWrapper.classList.add(
        "marker__wrapper",
        `--is-${slugify(markerData.label)}`
    );
    ...
    markerWrapper.onclick = () => {
        ...
        const cityIndex = Number(
            getComputedStyle(city).getPropertyValue("--inline-index")
        );
        const cityPosition = citiesInitialPosition[cityIndex - 1];

        if (
            cityIndex < 1 ||
            cityIndex > citiesInitialPosition.length ||
            !cityPosition
        ) {
            return;
        }
