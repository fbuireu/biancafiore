...
- **Modifiers are BEM modifiers** (ADR 0014): `block--modifier` or `block__element--modifier`, fused into a single class (`.contact-tab--active`, `.contact-form__submit--loading`, `.article-card__item--landscape`). No standalone `.--modifier` classes, and no `is-`/`has-` prefixes — that's SMACSS vocabulary, not BEM.
- **Cross-block behaviour is a block, mixed in.** Utilities used by unrelated blocks (`underline-on-hover`, `clickable`, `inverted-color-scheme`, `current-page`) live in `@styles/global/modifiers.css` as blocks of their own and are applied alongside the component's own block — a BEM *mix*: `class="navigation__menu__link underline-on-hover clickable"`. Reach for one of these before inventing a per-block duplicate. A block may still override the mix for itself — `menu.css` paints `.navigation__menu__link.current-page` gold — and because component stylesheets are unlayered they win over `@layer modifiers` without needing extra specificity.
