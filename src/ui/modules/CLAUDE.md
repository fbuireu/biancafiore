...
- **Cross-block behaviour is a block, mixed in.** Utilities used by unrelated blocks live in `@styles/global/modifiers.css` as blocks of their own and are applied alongside the component's own block — a BEM *mix*: `class="navigation__menu__link underline-on-hover clickable"`. A block may still override the mix for itself — `menu.css` paints `.navigation__menu__link.current-page` gold — and because component stylesheets are unlayered they win over `@layer modifiers` without needing extra specificity. Reach for one of these before inventing a per-block duplicate:
