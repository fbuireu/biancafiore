...
- **Cross-block behaviour is a block, mixed in.** Utilities used by unrelated blocks (`underline-on-hover`, `clickable`, `inverted-color-scheme`) live in `@styles/global/modifiers.css` as blocks of their own and are applied alongside the component's own block — a BEM *mix*: `class="navigation__menu__link underline-on-hover clickable"`. Reach for one of these before inventing a per-block duplicate. `current-page` is what that looks like when it goes wrong: it is mixed in like the others but declared twice, in `breadcrumbs.css` and `menu.css`, and belongs in `modifiers.css`.
