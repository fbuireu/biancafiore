...
- **BEM throughout** (ADR 0014). Modifiers are fused onto their block (`.theme-toggle--toggled`, `.reveal--fade`), never standalone `.--modifier` classes. `modifiers.css` holds the cross-block utilities that BEM treats as *blocks used as a mix* — `underline-on-hover`, `clickable`, `inverted-color-scheme`, `current-page` — applied next to a component's own block class.
