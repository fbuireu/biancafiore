...
The Vite CSS transformer stays LightningCSS, configured with `exclude: Features.LightDark` and `errorRecovery: true` in `astro.config.ts`. Excluding the feature emits native `light-dark()` untouched; error recovery keeps the rejected selector from breaking the build.
...
- The failure mode is silent in dev, which is why it is repeated as a Gotchas bullet in `CLAUDE.md` and in the styles guide instead of living only here.
