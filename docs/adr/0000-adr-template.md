...
- Where the decision bites in the rest of the docs: the Gotchas bullet, the nested guide, or the [`CONTEXT.md`](../../CONTEXT.md) entry that has to link back here. There is no index; an ADR nothing links to will not be read, and [`docs/docs-consistency.test.ts`](../docs-consistency.test.ts) fails on one that nothing links to.
