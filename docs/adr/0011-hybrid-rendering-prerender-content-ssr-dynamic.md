...
The adapter runs `output: "server"` (ADR 0001), but almost every page on the site is the same for every visitor and changes only when its Contentful entry does. Paying per-request SSR for those pages buys nothing; going fully static to avoid it would give up server actions, which the contact form needs.
