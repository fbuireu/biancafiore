...
- **Nothing in the code makes the requirement visible**, which is why it is a Gotchas bullet in [`AGENTS.md`](../../AGENTS.md) as well as here. A well-meaning change that reads `HIDE_CHROME` in a second module, or that adds a route to `PUBLISHED_ROUTES` to "unblock" a preview, publishes an unfinished page to a client's domain without breaking anything observable.
