...
- **Nothing in the code makes the requirement visible**, which is why it is a Gotchas bullet in [`CLAUDE.md`](../../CLAUDE.md) as well as here. A well-meaning change that reads `HIDE_CHROME` in a second module, or that adds a route to `PUBLISHED_ROUTES` to "unblock" a preview, publishes an unfinished page to a client's domain without breaking anything observable.
- **Removing it is a deletion, not a migration.** When every page is ready, `siteChrome` collapses to nothing and those questions go with it. Until then, adding a page to `PUBLISHED_ROUTES` is the act of publishing it, and should be reviewed as such.
