...
- **A content loader is reachable, at the price of two mocks per file.** `astro:content` is stubbed down to `defineCollection` returning its argument, and `CmsClientLive` is replaced with the stub layer in `testing/doubles/cmsLayer.ts` — swapping the layer rather than `runCms` keeps the real `ManagedRuntime`, the real `Effect.all` batching and the real `isContentfulConfigured` in the test. `articles` and `authors` are covered that way; `cities`, `projects`, `tags` and `testimonials` still are not.
