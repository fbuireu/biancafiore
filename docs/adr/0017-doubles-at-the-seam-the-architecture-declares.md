...
**`onUnhandledRequest: "error"` is not what enforces that, and must not be relied on.** MSW exempts any request whose pathname ends in an asset extension — its `isCommonAssetRequest` covers `.jpg`, `.png`, `.webp` and a dozen more, JSON and XML among them — from the unhandled-request strategy entirely, and performs it against the real network. Every URL `getImagePlaceholder` builds ends in one, so a placeholder test that lost its double called Contentful's CDN for real and still passed. What enforces it instead is a catch-all `http.all("*")` registered in `tests/doubles/network.ts` as the server's *initial* handler: it answers every unmatched request with a network error and pushes it onto `escapedRequests`, and `tests/setup/network.ts` throws in `afterEach` if that array is not empty. `resetHandlers` restores initial handlers, so the catch-all survives every reset; `server.use` prepends, so a real double always wins over it.
...
- **A test that forgets its double now fails loudly** rather than silently reaching the network, because the catch-all records it and the `afterEach` in `tests/setup/network.ts` turns that recording into a failure. That is the behaviour we want and it also means every new outbound call must be declared in a test before it can pass. It is deliberately *not* delegated to `onUnhandledRequest`, for the reason in the Decision.
