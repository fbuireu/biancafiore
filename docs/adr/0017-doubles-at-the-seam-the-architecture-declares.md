...
Two call sites do not. `verifyRecaptcha` posts to Google's siteverify endpoint and `getImagePlaceholders` fetches image derivatives, both with a bare `fetch`. There is no seam there at all: the URL, the method and the body are assembled inline.
