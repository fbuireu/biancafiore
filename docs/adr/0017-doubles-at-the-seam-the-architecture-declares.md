...
For anything behind an Effect layer, the double is a layer. [`src/tests/doubles/contactLayers.ts`](../../src/tests/doubles/contactLayers.ts) and `cmsLayer.ts` build stand-ins for `Database`, `EmailClient` and `CmsClient`. Dropping to HTTP for these would couple the suite to Contentful's and Resend's wire formats, which we do not control and cannot usefully assert; the layer is typed, so a contract change fails at compile time instead.
