...
- **The `ArticleSlider` placements repeat the same responsive ramp**, differing only in the final `--slides-per-view`. A default in `slider.css` plus one override per placement would say the same thing in a fraction of the CSS.
...
- **The e2e suite covers almost nothing on a preview.** Every contact spec skips itself under `HIDE_CHROME`, and the theme-toggle spec survives only because the footer renders on the placeholder. [`e2e/client-router-rewiring.spec.ts`](../e2e/client-router-rewiring.spec.ts) now covers the wiring a second pageview can break, and runs entirely inside `PUBLISHED_ROUTES` so `HIDE_CHROME` can never skip it, but everything a reader does on `/`, `/about`, `/contact` and `/projects` is still unreachable there. The manual `end-2-end-tests.yml` that could have run the suite against production was never dispatched and is deleted; the gap closes when `HIDE_CHROME` stops being `true` on the `development` environment, which is a settings change rather than a change here.
- **The open menu does not contain focus.** Escape works and returns focus to the button, so it is not a trap, but tabbing past the last menu link walks into content the overlay covers. `inert` on `main` and `footer` while `page--menu-open` is set is a small change.
...
- **`createImage` casts an asset it has not checked.** It takes `Asset | UnresolvedLink<"Asset">`, casts to the resolved half, casts `fields.file` past its optionality, and reads `details.image?.width as number`, which is `undefined` typed as `number` for an asset that is not an image (a PDF, say). The article and author mappers now refuse an unresolved *entry* link by narrowing on `"fields" in`, and this is the same union one layer down, unguarded. It is not the same risk: every caller passes an image field, and the domain `imageSchema` requires numeric dimensions, so the collection load fails on a non-image rather than shipping one. Worth the same treatment as `createAuthor` when it is next touched, since the cast is what hides which failure a reader would get.
- **The 500 page prints the error message into its public meta description.** `500.astro` builds `For full transparency this is the error: ${reference}` from `error.message` and hands it to `Seo.astro`, which renders it as `<meta name="description">`, `og:description` and `twitter:description`. The copy says the transparency is intended and the page is `noindex, nofollow`, so this is a decision rather than a slip, but it means an internal message reaches a public response body. Named here so it is a choice on the record rather than an accident. The response itself is not otherwise exposed: a throwing route was checked in dev on 2026-08-29 and its 500 carries the full security header set, so the middleware wraps the error page as it does any other.
