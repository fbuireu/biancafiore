...
- **`TestimonialsSlider` reimplements the slider.** It opts out with `hasButtons={false}` and then writes its own arrows, dots, active-index tracking and `scrollIntoView`, 47 lines beside `SliderShell`'s own 20. The two do not even agree on what "next" means: `SliderShell` pages by track width and disables at the ends, `TestimonialsSlider` centres a slide and wraps modulo. Collapsing it means either a `hasDots` prop next to `hasButtons`, or restoring the native `::scroll-marker` dots that every caller used to turn off. The modules guide already claims `TestimonialsSlider` "differs only in the class it hands it"; today that is false.
- **The JSON-LD image crops spell Contentful's Images API in `src/ui`.** `buildArticleImageVariants` writes `?w=&h=&fit=fill` by hand, which is the vocabulary `buildContentfulImageUrl` exists to own, and the docs test forbids `src/ui` importing `@infrastructure`, so the leak had nowhere else to go. Under the Cloudflare image service the crops also point at the Contentful origin while the page's `<img>` goes through `/cdn-cgi/image`. The fix is the DTO carrying the variants, which is a schema change.
- **A City's Period is stored as a rendered string.** `formatPeriod` emits `"2015-2018"` and `citiesSchema` stores it as `z.string()`, so nothing downstream can emit `<time datetime>`, sort by it, or tell "still there" from "left in 2018", and the cities loader has to ask Contentful for the order because the DTO no longer carries a sortable start date. `"NaN-Present"` also passes the schema and reaches the page. Storing `{ startYear, endYear? }` and rendering at the card is the concentrating move.
- **`ArticleCard` takes a whole `CollectionEntry` and is spread at four call sites.** `type Props = CollectionEntry<"articles"> & { isLandscape?: boolean }` puts a presentational flag in the same namespace as every CMS field, so adding `isLandscape` to `articleSchema` would silently collide, and it re-spreads the entire rendered article body into props for a card that shows a 200-character description.
- **`groupBy` is a domain rule wearing a generic name.** It sorts the keys and sorts each bucket by `.name`, neither of which its name says, and its `T extends Named` constraint is the tell. Its one consumer supplies the other half of the A-Z rule as an inline lambda in `pages/tags/index.astro`. `buildTagIndexBuckets` in `@domain/tag/rules.ts`, beside `resolveSlugCollisions`, would own the whole arrangement `CONTEXT.md` describes.
- **`createImage` is the only file under `src/shared/application/`** and it is typed over Contentful's `Asset`, which the application guide says may not happen outside `src/application`. Moving it to `src/application/dto/shared/images.ts` deletes three directory levels and a branch of the tree.
...
- **`initMenu` is deep but unreachable from a test.** It takes nothing and returns nothing, so a test cannot hand it a document or a fake timeline; all eight tests in `interactions.test.tsx` cover `backgroundObserver`, and the 140 lines that manage listener lifetime across router swaps have none. Splitting construction from wiring would make the `AbortController` contract assertable without flattening the module.
