...
**A manual dispatch redeploys, and with its `release` input checked it releases too.** Every credential is read at build or deploy time, so rotating one changes no file; a dispatch on `main` runs the production deploy with the smoke run behind it and, by default, cuts no release, since the same sha versions nothing. The input exists for one failure: a commit landing on `main` while the release job runs makes its version-bump push a non-fast-forward, the job fails after the deploy and the smoke run passed, no tag is written, and a re-run stands down because the branch it holds is behind the remote. A dispatch with `release` checked redeploys the head of `main`, smokes it and releases from a fresh checkout, over every commit since the last tag.
